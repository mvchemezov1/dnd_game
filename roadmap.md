graph TD
    ...
    subgraph Infrastructure["Инфраструктура"]
        I1[Event Store и Snapshot]
        I2[Message Bus: InMemory/RabbitMQ]
        I3[Безопасность: Auth/Токены]
    ...
    subgraph Frontend["Клиентская часть"]
        F1[Auth и UI-helpers]
        F2[Несоответствие API: DTO/поля]
        F3[Отсутствует websocket-client.js]
    ...
    subgraph SharedKernel["Общие компоненты"]
        ...
        S3[Дублирование базовых интерфейсов/утилит]
        S4[Мёртвый код: Result/Maybe/IRepository]
    end

    A2 -->|"Неинициализация полей, неполный RebuildAsync"| I1
    A3 -->|"Не передаётся количество, семантические ошибки"| P1
    A4 -->|"Необработанные события, некорректные проверки"| A1
    A5 -->|"Неверные награды, фиксированный hitDieType"| A4
    ...
    I7 -->|"Блокировка по SagaId, а не по CorrelationId"| I2
    ...
    S1 & S2 & S3 & M1 & F1 -->|"Дублирование/несогласованность"| Risk4(Поддерживаемость)
